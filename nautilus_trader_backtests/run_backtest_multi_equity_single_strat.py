...
# ---------------------------------------------------------------------------------
SYMBOLS         =   [
    "AAPL", "MSFT", "GOOG", "AMZN", "TSLA", "META", "NVDA", "NFLX", "JNJ",
    "V", "UNH", "JPM", "PG", "MA", "HD", "BAC", "XOM", "PFE", "DIS",
]
MULTIPLIERS     =   [1 / len(SYMBOLS) for _ in SYMBOLS]     # * 0-1 for percentage of investment
START_DATE      =   f"{current_year-1}-07-02"
END_DATE        =   f"{current_year-1}-12-31"
INTERVAL        =   "1h"
INVESTMENT      =   Decimal(750_000)
VENUE_BAL       =   "1_000_000 USD"
STRAT_INDEX     =   3                                       # 0 based index for strats in strategy_config.json
# ----------------------------------------------------------------------------------

sims = [TestInstrumentProvider.equity(symbol=s, venue="SIM") for s in SYMBOLS]

raw_key = f"{"".join(SYMBOLS)}{START_DATE}{END_DATE}{INTERVAL}"
hash_id = hashlib.sha1(raw_key.encode()).hexdigest()[:12]

CATALOG_PATH = Path().resolve() / "Data" / f"{hash_id}"
